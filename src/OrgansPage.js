...
const renderOrganCard = (organ, index, actions, totalCount) => {
  const relatedCount = organ.muscle_organs?.length || 0;
...
const OrgansPage = createEntityList({
  entityName: 'Орган',
  entityType: 'organ',
  tableName: 'organs',
  columns: [
  ...
  relatedTables: [
    { table: 'muscle_organs', fields: 'muscle_id' }
  ],
  statsConfig: {
    calculate: (entities) => ({
      total: entities.length,
      withRelations: entities.filter(o => (o.muscle_organs?.length || 0) > 0).length
    })
  },
